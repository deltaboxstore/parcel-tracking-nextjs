...
///   };
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    navigate(`/track-your-item?${params.toString()}`, { replace: false });
  ...
  const usZipRegex = /^\d{5}(-\d{4})?$/;
    ...
    setTrackingUrl(url);
  }, [trackingNumber, destinationCountry, postcode, orderNumber, fromDate, toDate, navigate, setCountrySpecificMessage, userCountry, allowedDestinations, t]); 
  ...
  return (
    ...
    <div className="modern-form-container">
      ...
      <br />
      <p>{t('clickLinkAbove') || 'Please click on the link above to track your parcel.'} <h2><u>{t('trackingDetailsNote') || 'Note that tracking details may not show up in search results, so it\'s recommended to paste the tracking number into the system to track the item.'}</u></h2></p>
      <br />
